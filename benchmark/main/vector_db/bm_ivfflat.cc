...
int main(int argc, char **argv) {
    ...
    auto build_blob_index = [&]() -> std::unique_ptr<VectorIndex> {
      switch (indexType) {
    ...
    auto build_base_index = [&]() -> std::unique_ptr<BaseVectorIndex> {
      switch (indexType) {
      case IndexType::IVFFlat: {
    ...
    std::unique_ptr<VectorIndex> blob_index = nullptr;
    std::unique_ptr<BaseVectorIndex> base_index = nullptr;
    ...
    leanstore->worker_pool.ScheduleSyncJob(0, [&]() {
        ...
        blob_index = build_blob_index();
        leanstore->CommitTransaction();
    });

    // auto blob_index_end = std::chrono::high_resolution_clock::now();
    // print_timing("Blob Index building", blob_index_start, blob_index_end);

    // if (FLAGS_benchmark_baseline) {
    //     auto baseline_index_start = std::chrono::high_resolution_clock::now();
    //     index_vec.build_index_vec();
    //     auto baseline_index_end = std::chrono::high_resolution_clock::now();
    //     print_timing("Baseline Index building", baseline_index_start, baseline_index_end);
    // }
    ...
    //     print_timing("Blob Index lookup", blob_lookup_start, blob_lookup_end);
